...
costs = [2.75, 2.92, 3.09, 3.25, 3.42]
...
for i in range(0,3):    
    df = pd.read_excel(f'{path}/MOD_{rsmlist[i]}_ndSortRes.xlsx',header=None).iloc[:,[-4,-3]].astype(float).values           
    ax1.scatter(df[:, 0], -df[:, 1], s=60, c='none',alpha=0.8, 
                marker=markers[i], edgecolors=colors[i],label=labels[i])


ax1.set_xlim(2.4,5.6)
ax1.set_xticks(np.linspace(2.6,5.4,5,endpoint=True))
