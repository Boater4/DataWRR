...
for num in range(0,30): 
            ...
            for i in range(1,37): 
                ...
                for j in range(0,58): 
                    ...
                    if toNode[j] == i and tk.ENgetlinkvalue(j+1, 0) > 0.01:
                        nPipe += 1
                        pipeIndex[nPipe-1] = 1+j
                        if fromNode[j] != 37:   tempNei.append(fromNode[j]) #排除水源Index=37
    ...
    with open('OptRes/FOS_NRI2u_RunTime.csv', 'a', newline='') as file:
    ...
    XF.to_csv(f"OptRes/FOS_NRI2u_seed{num}.csv")   #导出决策变量和目标函数
...
nXF.to_csv("opt30ResFOS/FOS_NRI2u_opt30.csv")   #导出决策变量和目标函数  
