...
% 首先，指定包含csv文件的文件夹
filePath = strcat(new_filePath,'\opt30ResPES');
...
for i = 1:length(fileNameList)
    ...
    f=non_domination_sort_mod(unique_all_pf,2,99);
    best_pf=f(f(:,102)==1,:);
    
    subplot(2,4,i);
    scatter(unique_all_pf(:,100),-unique_all_pf(:,101),15,'b');
    hold on;
    scatter(best_pf(:,100),-best_pf(:,101),15,'r','filled');

    ylabel(rsm+'(-)');
    xlabel('Cost(M€)');
        
    writeFileName = strcat(new_filePath,'\ndSortResPES\PES_',rsm,'_ndSortRes.xlsx');
	sheetName = 'best_pf';
