...
# diameterList_mm = [np.float32(i*25.4) for i in diameterList]

files = os.listdir('ndSortResHAN')


for j in files[0:1]:
    rsm = j.split("_")[1]
    df = pd.read_excel(f"ndSortResHAN/{j}",header=None) #输入xlsx无表头
    print(len(df))
    solution = df.iloc[:,0:34].astype(float).values
    cost = df.iloc[:,34].astype(float).values
    
    sinMRS = []
    cmulMRS = []
    overMRS = []
    for i in range(0,len(solution)): 
        wn = wntr.network.WaterNetworkModel("networks/HAN.inp")
        ...
        for pipe_name, pipe in wn.pipes():
            pipe.diameter = diameter_list[k]*0.0254 #wntr的管径用m作单位
            k += 1
        # wn.write_inpfile(f'all_sol/HAN_AllSol_MRS/HAN_{metric}_Sol{i}.inp', version=2.2)
        ...
        # Calculate single pipe failure scenarios 
        MRScore = 0
        cp = 0
        omitList = ['1','2'] # Pipe 1 and 2 excluded 
        for pipe_name in wn.pipe_name_list: 
            if pipe_name in omitList:
                continue
    ...
    df.to_excel(f'paper2_HAN_MRSres/{rsm}_MRSres.xlsx')
