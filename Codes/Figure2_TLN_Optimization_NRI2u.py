...
for num in range(0,nRuns):  
    ...
    class TLNProblem(ElementwiseProblem):
        ...
        def _evaluate(self, x, out, *args, **kwargs):
            ...
            # Calculate diameter uniformity coefficient for NRI2u
            ...
            for i in range(1,7): 
                ...
                for j in range(0,8): 
                    ...
                    if toNode[j] == i and tk.ENgetlinkvalue(j+1, 0) > 0.01:
                        nPipe += 1
                        pipeIndex[nPipe-1] = 1+j 
                        if fromNode[j] != 7:   tempNei.append(fromNode[j]) 
    ...
    plt.title("Cost VS NRI2u", fontsize=18)
    plt.show()
    ...
    F_res.columns = ['Cost','NRI2u']
    XF = pd.concat([X_res,F_res],axis=1)
    # XF.to_csv(f"TLN_NRI2u_run{num}_gen{nGens}.csv")   
    nXF = pd.concat([nXF,XF],axis=0) 
    nXF = nXF.drop_duplicates(keep=False)

nXF.to_excel(f"TLN_NRI2u_run{nRuns}_gen{nGens}.xlsx", index=False)   
