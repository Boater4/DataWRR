...
files = os.listdir('ndSortResPES')


for j in files[:]:
    rsm = j.split("_")[1]
    df = pd.read_excel(f"ndSortResPES/{j}",header=None) #输入xlsx无表头
    print(len(df))
    solution = df.iloc[:,0:99].astype(float).values
    cost = df.iloc[:,99].astype(float).values
    ...
    for i in range(0,len(solution)): 
        ...
        for pipe_name, pipe in wn.pipes():
            ...
            k += 1
        # wn.write_inpfile(f'all_sol/PES_AllSol_MRS/PES_{metric}_Sol{i}.inp', version=2.2)
    ...
    df.to_excel(f'paper2_PES_MRSres/{rsm}_MRSres.xlsx')
