...
wn = wntr.network.WaterNetworkModel('MODinp/MOD_NRI_Sol130.inp')
diaList = []
for name, pipe in wn.pipes():
    diaList.append(1000*float(f'{pipe.diameter:.3f}'))
NRI = diaList

wn = wntr.network.WaterNetworkModel('MODinp/MOD_NRI2u_Sol227.inp')
diaList = []
for name, pipe in wn.pipes():
    diaList.append(1000*float(f'{pipe.diameter:.3f}'))
NRI2u = diaList

wn = wntr.network.WaterNetworkModel('MODinp/MOD_NRI2uk_Sol137.inp')
diaList = []
for name, pipe in wn.pipes():
    diaList.append(1000*float(f'{pipe.diameter:.3f}'))
NRI2uk = diaList
...
plt.ylim(90,610)
plt.yticks(diameters[:11],fontsize=17)
...
plt.tight_layout()
plt.savefig('******/MOD_diaDis.svg', dpi=600)  
