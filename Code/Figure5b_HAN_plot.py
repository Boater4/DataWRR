...
costs = [6.46, 6.81, 7.16, 7.50, 7.85]
...
for i in range(0,3):    
    df = pd.read_excel(f'{path}/HAN_{rsmlist[i]}_ndSortRes.xlsx',header=None).iloc[:,[-4,-3]].astype(float).values           
    ax1.scatter(df[:, 0], -df[:, 1], s=60, c='none',alpha=0.8, 
                marker=markers[i], edgecolors=colors[i],label=labels[i])


ax1.set_xlim(5.9,11.2)
ax1.set_xticks(np.linspace(6,11,6,endpoint=True))
ax1.set_ylim(0.18,0.38)
ax1.set_yticks(np.linspace(0.2,0.36,5,endpoint=True))
...
for i in range(3,4):
    df = pd.read_excel(f'{path}/HAN_{rsmlist[i]}_ndSortRes.xlsx',header=None).iloc[:,[-4,-3]].astype(float).values           
    ax1.scatter(df[:, 0], -df[:, 1], s=60, c='none',alpha=0.5, 
                marker=markers[i], edgecolors=colors[i],label=labels[i])
