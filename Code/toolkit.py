...
class ENepanet:
    ...
    def _error(self):
        ...
        # logger这一行我隐藏了，为了不一直显示EPANET error: 6
...
#            raise EpanetException("EPANET Error {}".format(self.errcode))
    ...
    # 2023.10.7 self-written------------
    def ENgetlinknodes(self,sId):
        iIndex = ctypes.c_int()
        jIndex = ctypes.c_int()
        if self._project is not None:
            self.errcode = self.ENlib.EN_getlinknodes(self._project, sId, byref(iIndex), byref(jIndex))           
        else:
            self.errcode = self.ENlib.EN_getlinknodes(sId, byref(iIndex), byref(jIndex)) 
        self._error()
        return iIndex.value, jIndex.value        
    #-----------------------------------
