...
costs = [0.03, 0.06, 0.09, 0.12, 0.15]
...
for i in range(0,3):    
    df = pd.read_excel(f'{path}/FOS_{rsmlist[i]}_ndSortRes.xlsx',header=None).iloc[:,[-4,-3]].astype(float).values           
    ax1.scatter(df[:, 0], -df[:, 1], s=60, c='none',alpha=0.8, 
                marker=markers[i], edgecolors=colors[i],label=labels[i])

ax1.set_xlim(0.0,0.46)
ax1.set_xticks(np.linspace(0.03,0.43,5,endpoint=True))
