...
for num in range(0,30): 
    ...
    class MODProblem(ElementwiseProblem):
        ...
        def _evaluate(self, x, out, *args, **kwargs):
            ...
            # for i in range(0,len(wn.pipe_name_list)):
                ...
                # pipe.diameter = float(diameterList[var]*0.001) #MOD与HAN单位不同，但输入WNTR中都是SI单位
    ...
    with open('OptRes/MOD_NRI_RunTime.csv', 'a', newline='') as file:
    ...
    XF.to_csv(f"OptRes/MOD_NRI_seed{num}.csv")   #导出决策变量和目标函数
...
nXF.to_csv("opt30ResMOD/MOD_NRI_opt30.csv")   #导出决策变量和目标函数  
